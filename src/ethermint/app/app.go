...
// #stable - 0.4.0
func (app *EthermintApplication) Info(_ abciTypes.RequestInfo) abciTypes.ResponseInfo {
	blockchain := app.backend.Ethereum().BlockChain()
	currentBlock := blockchain.CurrentBlock()
	height := currentBlock.Number()
	hash := currentBlock.TxHash()

	log.Println("Info", "height", height) // nolint: errcheck
...
// #stable - 0.4.0
func (app *EthermintApplication) SetOption(r abciTypes.RequestSetOption) abciTypes.ResponseSetOption {
	log.Println("SetOption", "key", r.Key, "value", r.Value) // nolint: errcheck
	return abciTypes.ResponseSetOption{}
}

// InitChain initializes the validator set
// #stable - 0.4.0
func (app *EthermintApplication) InitChain(r abciTypes.RequestInitChain) abciTypes.ResponseInitChain {
	log.Println("InitChain") // nolint: errcheck
...
func (app *EthermintApplication) CheckTx(txBytes []byte) abciTypes.ResponseCheckTx {
	...
	if err != nil {
		log.Println("CheckTx: Received invalid transaction", "tx", tx)
		return abciTypes.ResponseCheckTx{Code: 2, Log: err.Error()}
	}
	log.Println("CheckTx: Received valid transaction", "tx", tx) // nolint: errcheck
...
func (app *EthermintApplication) DeliverTx(txBytes []byte) abciTypes.ResponseDeliverTx {
	...
	if err != nil {
		// nolint: errcheck
		log.Println("DelivexTx: Received invalid transaction", "tx", tx, "err", err)
		return abciTypes.ResponseDeliverTx{Code: 2, Log: err.Error()}
	}
	log.Println("DeliverTx: Received valid transaction", "tx", tx) // nolint: errcheck

	res := app.backend.DeliverTx(tx)
	if res.IsErr() {
		// nolint: errcheck
		log.Println("DeliverTx: Error delivering tx to ethereum backend", "tx", tx,
			"err", err)
		return res
	}
	app.CollectTx(tx)
...
// #stable - 0.4.0
func (app *EthermintApplication) BeginBlock(r abciTypes.RequestBeginBlock) abciTypes.ResponseBeginBlock {
	log.Println("BeginBlock") // nolint: errcheck

	// update the eth header with the tendermint header
	app.backend.UpdateHeaderWithTimeInfo(&r.Header)
	return abciTypes.ResponseBeginBlock{}
}

// EndBlock accumulates rewards for the validators and updates them
// #stable - 0.4.0
func (app *EthermintApplication) EndBlock(r abciTypes.RequestEndBlock) abciTypes.ResponseEndBlock {
	log.Println("EndBlock", "height", r.Height) // nolint: errcheck
...
func (app *EthermintApplication) Commit() abciTypes.ResponseCommit {
	log.Println("Commit") // nolint: errcheck
	blockHash, err := app.backend.Commit(app.Receiver())
	if err != nil {
		// nolint: errcheck
		log.Println("Error getting latest ethereum state", "err", err)
		return abciTypes.ResponseCommit{Data: blockHash.Bytes()}
	}
	state, err := app.getCurrentState()
	if err != nil {
		log.Println("Error getting latest state", "err", err) // nolint: errcheck
...
func (app *EthermintApplication) Query(query abciTypes.RequestQuery) abciTypes.ResponseQuery {
	log.Println("Query") // nolint: errcheck
	var in jsonRequest
	if err := json.Unmarshal(query.Data, &in); err != nil {
		return abciTypes.ResponseQuery{Code: 2, Log: err.Error()}
	}
...
func (app *EthermintApplication) validateTx(tx *ethTypes.Transaction) abciTypes.ResponseCheckTx {
	...
	if tx.Size() > maxTransactionSize {
		return abciTypes.ResponseCheckTx{Log: core.ErrOversizedData.Error(), Code: 2}
	}
	...
	if err != nil {
		return abciTypes.ResponseCheckTx{Log: core.ErrInvalidSender.Error(), Code: 2}
	}

	// Transactions can't be negative. This may never happen using RLP decoded
	// transactions but may occur if you create a transaction using the RPC.
	if tx.Value().Sign() < 0 {
		return abciTypes.ResponseCheckTx{Log: core.ErrNegativeValue.Error(), Code: 2}
	}
	...
	if gasLimit.Cmp(tx.Gas()) < 0 {
		return abciTypes.ResponseCheckTx{Log: core.ErrGasLimitReached.Error(), Code: 2}
	}

	// Check if nonce is not strictly increasing
	nonce := currentState.GetNonce(from)
	if nonce != tx.Nonce() {
		return abciTypes.ResponseCheckTx{Log: fmt.Sprintf("Nonce not strictly increasing. Expected %d Got %d",
			nonce, tx.Nonce()), Code: 2}
	}
	...
	if currentBalance.Cmp(tx.Cost()) < 0 {
		return abciTypes.ResponseCheckTx{Log: fmt.Sprintf("Current balance: %s, tx cost: %s",
			currentBalance, tx.Cost()), Code: 2}
	}

	intrGas := core.IntrinsicGas(tx.Data(), tx.To() == nil, true) // homestead == true
	if tx.Gas().Cmp(intrGas) < 0 {
		return abciTypes.ResponseCheckTx{Log: core.ErrIntrinsicGas.Error(), Code: 2}
	}
